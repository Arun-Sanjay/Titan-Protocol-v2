...
export default function EmailLoginScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setBusy(true);
    // Phase 4.3: track whether sign-in succeeded so we can keep the
    // loading state until the root layout redirects. Previously, the
    // finally block always called setBusy(false), which reset the form
    // to idle for 100-500ms before the redirect fired — the user saw
    // the form flash back and it felt like nothing happened.
    let succeeded = false;
    try {
      if (mode === "password") {
        ...
        // Phase 4.3: directly update the auth store with the returned
        // session for instant redirect. The old code relied solely on
        // onAuthStateChange → store update → root layout re-render, which
        // takes 100-500ms and makes the login feel unresponsive.
        if (data.session) {
        ...
        succeeded = true;
        // Root layout will redirect to /(tabs) on next render.
      } else {
      ...
      // Keep the loading spinner showing if sign-in succeeded — the
      // redirect will unmount this screen. Only reset on failure.
