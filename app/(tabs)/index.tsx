...
export default function HQScreen() {
  ...
  // Phase 4.3: In-flight guard prevents concurrent mutation chains when
  // the user rapidly taps tasks. Each toggle fires toggleCompletion →
  // awardXP → updateStreak → maybeEnqueueRankUp — without a guard,
  // 8 rapid taps create 8 overlapping async chains with cascading
  // re-renders and query invalidations that can freeze/crash the app.
  ...
  const handleToggle = useCallback(
    async (taskId: string) => {
      // Phase 4.3: skip if this task's mutation chain is already running.
      ...
      try {
        ...
        logError("HQScreen.handleToggle", e, { taskId });
