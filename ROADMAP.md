...
| 3.3 | Data layer refactor (services + hooks for 5 domains) | ✅ Done (infra complete, consumer cutover in 3.5) | 2026-04-07 · React Query infra + profile/tasks/habits/protocol/rank-ups services + hooks |
| 3.4 | Offline-first sync (mutation queue + banner + resume sync) | ✅ Done | 2026-04-07 · networkMode offlineFirst, OfflineBanner, AppState resume |
| 3.5 | MMKV → Supabase migration + consumer cutover | ✅ Done (core consumers) | 2026-04-07 AM · migration script + engine detail + add-task + rank-up overlay. 2026-04-07 PM · full consumer cutover sweep: dashboard, protocol screen, track tab (habits+journal XP), profile tab, achievement popup, titan unlock modal, hub/command, hub/focus, hub/workouts, hub/settings, skill-tree-evaluator lib helper. Remaining legacy consumers are intentional scope for 3.7 (analytics sparklines, engines tab, hub/analytics history, morning mission preview subcomponent) or gated on non-core service layer (Phase 3.8). |
| 3.6 | Animation leak sweep (Phase 2.1A v2) | ✅ Done | 2026-04-07 · 18 files with infinite withRepeat and no cancelAnimation cleanup (dashboard, skill-tree, 8 onboarding Beat* components, 4 story cinematics, walkthrough summary, titan unlock celebration, title wall, integrity warning, daily briefing, rank promotion). Same bug family as the Phase 2.1A 15+ task crash, crept in since that fix. |
| 3.7 | Native config blockers | ✅ Done | 2026-04-07 · babel.config.js + metro.config.js + babel-preset-expo hoist, POST_NOTIFICATIONS permission, versionCode strategy (gradle property + CI injection via github.run_number), R8 minify + resource shrink for release builds |
| 3.8 | Dependency hygiene + dead code sweep | ✅ Done | 2026-04-07 · expo install --check fixes, react-native-worklets downgrade to SDK 55 pin, deleted 1800 lines of dead code (src/components/v2/protocol/, 3 stub hooks, src/lib/protocol-completion.ts) |
| 3.9 | Realtime multi-device sync | 🚫 Deferred post-launch | |
...
- **2026-04-07 (evening)** — **Full-repo audit + remediation pass.** A comprehensive audit at the request of the user flagged 5 batches of problems that the earlier Phase 3.5 completion note had understated or missed. All 5 batches landed across 8 commits: (1) Animation leak sweep — 18 files had `withRepeat(..., -1, ...)` without `cancelAnimation` cleanup, including the dashboard itself and all 8 `src/components/v2/onboarding/Beat*.tsx` / `Step*.tsx` files. Phase 2.1A only fixed the original 8 offenders from CLAUDE.md's known-bugs list; new ones had crept in unnoticed since. (2) Native config blockers — created `babel.config.js` + `metro.config.js` (project had neither; Expo was auto-detecting through fragile fallbacks), hoisted `babel-preset-expo` to top-level devDependencies so babel could resolve it, added `POST_NOTIFICATIONS` to AndroidManifest.xml (silently broke notifications on Android 13+ before), rewrote versionCode to read from `TITAN_VERSION_CODE` gradle property with CI injection via `github.run_number` so Play Store updates can actually ship, updated CI workflow to pass the property through to `assembleRelease`. (3) Phase 3.5 consumer cutover — the ROADMAP entry for 3.5 admitted "dashboard task list still uses useEngineStore via the bridge adapter, Track tab habits screen, profile tab XP/level reads" were deferred. The audit found this meant ~97% of screens were still dual-writing to MMKV shadow state. Fixed by migrating every core consumer: `app/(tabs)/index.tsx` (dashboard tasks/completions/profile/protocol/rank-ups), `app/protocol.tsx` (morning+evening saves via `useSaveMorningSession`/`useSaveEveningSession`), `app/(tabs)/track.tsx` (habits reads+writes, journal XP), `app/(tabs)/profile.tsx` (XP/level/streak reads), `app/(modals)/achievement-popup.tsx` (achievement XP reward), `app/(modals)/titan-unlock.tsx` (XP read), `app/hub/command.tsx` (full cloud cutover — reads, toggles, XP), `app/hub/focus.tsx` (focus session XP), `app/hub/workouts.tsx` (workout XP), `app/hub/settings.tsx` (profile display), plus `src/lib/skill-tree-evaluator.ts`'s `checkHabitStreak()` lib helper which now reads `profiles.streak_current` via the shared `queryClient` singleton instead of the stale `useProtocolStore`. Removed the `handleToggleStr` string→number bridge in the dashboard. MissionRow now uses Supabase UUIDs throughout. (4) Dependency hygiene — `expo install --check` flagged 17 out-of-date packages including the critical `react-native-worklets@0.7.4` that was ahead of the SDK 55 pin at 0.7.2 and could cause Reanimated 4 worklet compile failures on Android. Resolved with `npm install --legacy-peer-deps` to work around a circular peer between expo-router 55.0.11 and @expo/log-box. Enabled `android.enableMinifyInReleaseBuilds` + `android.enableShrinkResourcesInReleaseBuilds` in gradle.properties (~30% APK size reduction). (5) Dead code — deleted `src/components/v2/protocol/` (6 files, 1162 lines, zero consumers), `src/lib/protocol-completion.ts` (~550 lines, only consumed by the dead v2/protocol/PhaseScore.tsx), and 3 stub hooks (`src/hooks/useProtocol.ts`, `useMindExercise.ts`, `useSkillProgress.ts` — all TODO placeholders with zero consumers). Total ~1800 lines removed. `tsc --noEmit` → 0 errors throughout, 67/67 Jest tests pass at every commit. **Not yet migrated (intentional follow-up):** `app/(tabs)/engines.tsx` read-only task list, `app/hub/analytics.tsx` historical 84-day sparklines/heatmap (via `useAnalyticsData` which still reads MMKV for historical data), the MorningMissionPreviewPhase + EveningScoreRevealPhase subcomponents in `app/protocol.tsx`, the 14-day `HabitChain` component (temporarily hidden in the Track tab until it's refactored to take logs as a prop). All non-core domains (gym, nutrition, sleep, weight, money, budget, deep-work, journal content, quests, achievements state, skill tree progress state, narrative, story, surprise, field-ops) remain on MMKV because they have no cloud service layer yet — that's a separate Phase 3.9+ pass.
...
- **2026-04-07** — **Phase 3.5 complete (data migration + core consumer cutover).** The cloud cutover is real. (a) `src/lib/migrate-to-supabase.ts` is a one-time MMKV → Supabase data migration that runs once per device per user the first time they open the cloud-enabled version while signed in. Migrates 7 domains (profile XP/level/streak, tasks, completions, habits, habit_logs, protocol_sessions, rank_up_events) in dependency order with per-domain try/catch isolation, batched upserts (500 rows at a time for completions/logs) using `ignoreDuplicates` for safe re-runs, numeric→UUID id maps preserved across the tasks→completions and habits→logs steps, and a `migration_to_supabase_completed` MMKV flag for idempotency. Coalesces three legacy protocol storage shapes into a single row per (user, date) — finally fixes the Phase 2.2A multi-key write race for migrated data too. ProgressCallback emits step + completedSteps + label so the UI can show progress. (b) `src/components/MigrationGate.tsx` wraps the authenticated render path, calls `maybeRunMigration()` on mount, and shows a cinematic-themed progress modal with a shimmer-animated progress bar until done. Mounted in `app/_layout.tsx` between `SystemNotificationProvider` and `OnboardingGate` so migration runs BEFORE the gate reads `profile.onboarding_completed`. Failure-tolerant: if migration errors, we still let the user into the app. (c) Engine detail screen `app/engine/[id].tsx` migrated to React Query hooks — `useEngineTasks`, `useEngineCompletions`, `useToggleCompletion` (optimistic + rollback), `useCreateTask`, `useDeleteTask`, `useAwardXP`, `useUpdateStreak`, `useEnqueueRankUp`. Score now computed via `computeEngineScore` from the service layer instead of derived in the store. `MissionRow.taskId` prop API changed from `number` to `string` to accommodate Supabase UUIDs; the dashboard's still-legacy callsite gets a temporary `handleToggleStr` adapter that coerces. (d) `app/(modals)/add-task.tsx` switched to `useCreateTask` with try/catch + Alert error handling. New `src/components/RankUpOverlayMount.tsx` is the cloud-backed `LevelUpOverlay` mount — reads from `usePendingRankUps` + `useDismissRankUp`, lives inside the `QueryClientProvider`. The Phase 2.1E inline `useProfileStore.pendingRankUps[0]` rendering in `app/_layout.tsx` is replaced by `<RankUpOverlayMount />`. Cross-device rank-ups now work — a level-up on device A appears on device B on next reconnect. tsc zero errors, 67/67 Jest tests pass. **Not yet migrated (intentional, post-launch follow-up):** dashboard task list (still uses `useEngineStore` via the bridge adapter), Track tab habits screen, profile tab XP/level reads, all non-core domains (gym/nutrition/sleep/weight/money/journal/quests/achievements/skill tree/narrative/story/surprise) remain Zustand+MMKV.
- **2026-04-07** — **Phase 3.4 complete.** Offline-first sync layer is in place. (a) `src/lib/query-client.ts` mutation defaults gain `networkMode: 'offlineFirst'` so writes pause when the device is offline instead of failing fast. The persister already serializes paused mutations to MMKV (Phase 3.3a), so they survive force-kill. The NetInfo listener now also calls `queryClient.resumePausedMutations()` on every online transition to fire queued writes in order. Cache buster bumped to `v1-phase3.4`. Conflict resolution: `updated_at` triggers on every mutable table from Phase 3.1 enable last-write-wins; subscription/rank-up tables are read-only or append-only from the client side so they can't conflict. (b) `src/hooks/useIsOnline.ts` is a thin wrapper around React Query's `onlineManager` so any component can subscribe without a direct NetInfo dep. New `src/components/ui/OfflineBanner.tsx` renders a thin status bar at the top of the screen when offline — slides in/out with a 220ms ease-out, `pointerEvents=none` so it never swallows taps, mounted in `app/_layout.tsx` at priority level 0. (c) `src/hooks/useAppResumeSync.ts` subscribes to `AppState 'change'` events and invalidates the 4 critical React Query caches (profile, today's completions, today's habit logs, pending rank-ups) when the app returns from background. 30s throttle prevents hammering Supabase on rapid app-switching. Mounted via the trivial `src/components/AppResumeSyncMount.tsx` zero-render component because the hook needs to live inside `QueryClientProvider`. tsc clean.
- **2026-04-07** — **Phase 3.3 complete (infrastructure + services; consumer cutover deferred to 3.5).** Cloud data layer is now available alongside the existing MMKV-backed Zustand stores — consumers can migrate domain by domain when it's safe to do so. 6 sub-phases across 6 commits: (a) React Query 5.96 + persist-client + async-storage-persister + NetInfo installed. `src/lib/query-client.ts` creates the QueryClient with sensible RN defaults (5min staleTime, 24h gcTime, 2 retries, refetchOnReconnect), an MMKV-backed AsyncStorage adapter for cache persistence, and a NetInfo → React Query onlineManager bridge. QueryClientProvider wraps both render paths in `app/_layout.tsx`. (b) `src/services/profile.ts` + `src/hooks/queries/useProfile.ts` with getProfile/updateProfile/upsertProfile/awardXP (returns `{ profile, leveledUp, fromLevel, toLevel }`)/updateStreak/completeOnboarding. New `src/components/OnboardingGate.tsx` uses `useProfile()` to redirect signed-in users to `/onboarding` if `!profile.onboarding_completed` — closes the Phase 3.2 deferral. (c) `src/services/tasks.ts` + `src/hooks/queries/useTasks.ts` for tasks + completions. listTasks/listAllTasks/listCompletions/toggleCompletion with OPTIMISTIC updates + rollback on the useToggleCompletion mutation. computeEngineScore helper (main=2pt, secondary=1pt). Soft delete on deleteTask (is_active=false). (d) `src/services/habits.ts` + hooks. toggleHabit recomputes current_chain/best_chain on the server round-trip using the Phase 3.1 denormalized columns — no more O(n²) scans. Optimistic log-for-date cache flip for instant HabitChain feedback. (e) `src/services/protocol.ts` + hooks. Single upsert per (user_id, date_key) via the unique constraint — the Phase 2.2A write-ahead flag becomes unnecessary because there's nothing to half-write. (f) `src/services/rank-ups.ts` + hooks backed by the `rank_up_events` table with the partial index on `WHERE dismissed_at IS NULL`. Replaces the Phase 2.1E MMKV queue: now cross-device, and dismissed events are soft-deleted for Phase 4.4 PostHog analysis. tsc --noEmit zero errors, 67/67 Jest tests pass. **Not yet migrated (intentional):** consumers (HQ dashboard, engine screen, Track tab, protocol screen) still read from the old Zustand stores; will cut over in Phase 3.5 alongside the one-time MMKV → Supabase data migration. Also not migrated in this pass: the non-core domains (gym, nutrition, sleep, weight, money, journal, quests, achievements, skill tree, narrative, story, surprise) — these remain Zustand+MMKV for now and migrate in a follow-up before launch.
