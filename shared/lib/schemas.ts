 ...
 */
import { z } from "zod";
// Platform-agnostic error logging (each app can override via setErrorLogger)
let _logError = (tag: string, error: Error, meta?: Record<string, unknown>) => {
  console.error(`[${tag}]`, error.message, meta);
};

export function setErrorLogger(
  fn: (tag: string, error: Error, meta?: Record<string, unknown>) => void,
) {
  _logError = fn;
}

function logError(tag: string, error: Error, meta?: Record<string, unknown>) {
  _logError(tag, error, meta);
}
