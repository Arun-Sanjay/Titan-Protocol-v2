...
export function DecisionDrill({ exercise, onComplete }: Props) {
      ...
      {revealed && (
        <Animated.View entering={FadeIn.duration(400)} style={styles.explanationWrap}>
          <Panel style={styles.explanationPanel}>
            <Text style={styles.explanationTitle}>{isCorrect ? "Correct!" : "Not quite."}</Text>
            <Text style={styles.explanation}>{exercise.explanation}</Text>
          </Panel>
          {exercise.insight && <Text style={styles.insight}>{exercise.insight}</Text>}
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  letterBadgeSelected: { backgroundColor: "rgba(255, 255, 255, 0.15)" },
  ...
  optionDesc: { fontSize: 11, color: colors.textMuted },
  explanationWrap: { marginTop: spacing.lg, gap: spacing.md },
  explanationPanel: { padding: spacing.lg },
  explanationTitle: { fontSize: 14, fontWeight: "700", color: colors.text, marginBottom: spacing.sm },
  explanation: { fontSize: 14, fontWeight: "400", color: colors.textSecondary, lineHeight: 22 },
  insight: { fontSize: 13, fontWeight: "400", fontStyle: "italic", color: colors.textMuted, paddingHorizontal: spacing.sm },
