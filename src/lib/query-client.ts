/**
 * Phase 3.3a: React Query client + MMKV-backed persister.
 *
 * React Query is the new data layer for Supabase reads/writes. It provides:
 ...
 *   - The old MMKV layer becomes the offline cache for the new data layer
 *     (Phase 3.4 adds mutation queue on top)
 *
...
export const queryClient = new QueryClient({
  defaultOptions: {
    ...
    mutations: {
      // Same retry policy for mutations. If the user toggles a task while
      // offline, the mutation retries until it lands (Phase 3.4 adds the
      // full mutation queue with resume-on-reconnect).
      retry: 2,
      retryDelay: (attempt) => Math.min(1000 * 2 ** attempt, 30_000),
    },
  },
...
// RN (the app lifetime == QueryClient lifetime).
persistQueryClient({
  queryClient,
  persister,
