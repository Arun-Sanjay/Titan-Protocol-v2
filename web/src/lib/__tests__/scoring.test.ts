...
describe("computeMonthConsistency", () => {
  ...
  it("resets the window after 30+ consecutive idle days", () => {
    // Active Mar 1-3 (100%), idle Mar 4 → Apr 2 (30 idle days), active Apr 3-5.
    // The 30-day gap should reset the window — only Apr 3-5 count.
    const scoreMap: Record<string, number> = {
      "2024-03-01": 100, "2024-03-02": 100, "2024-03-03": 100,
      "2024-04-03": 100, "2024-04-04": 100, "2024-04-05": 100,
    };
    ...
    expect(result.consistentDays).toBe(3);
    expect(result.consistencyPct).toBe(100);
    expect(result.currentStreak).toBe(3);
  });

  it("does not reset when idle run is below the threshold", () => {
    // 29 idle days — one short of the 30-day reset. Window should NOT reset.
    const scoreMap: Record<string, number> = { "2024-03-01": 100, "2024-03-31": 100 };
    const result = computeMonthConsistency(
      scoreMap,
      "2024-03-01",
      "2024-03-31",
  ...
  it("ignores the reset rule when resetAfterIdleDays = 0", () => {
    // Same scenario as the 30-day reset test, but reset is disabled.
    const scoreMap: Record<string, number> = {
      "2024-03-01": 100, "2024-03-02": 100, "2024-03-03": 100,
      "2024-04-03": 100, "2024-04-04": 100, "2024-04-05": 100,
    };
    ...
    expect(result.daysElapsed).toBe(36);
    expect(result.consistentDays).toBe(6);
  });
});
