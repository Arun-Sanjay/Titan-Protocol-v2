/**
 * Subscription / entitlement read-layer (P5 foundation).
 *
 * The `subscriptions` table is populated by the billing provider's webhook
 * (RevenueCat or Stripe — the schema is provider-shaped but this read side
 ...
 * downstream (feature gating, the Pro badge) keys off `deriveEntitlement`.
 *
 * Checkout + the webhook that writes this table are the user-action piece
 * (needs the provider's API keys) — see RELEASE_READINESS.md P5.
 *
