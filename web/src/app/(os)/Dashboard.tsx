...
import { EMPTY_SCORE, ENGINES, type EngineKey } from "@/lib/scoring";
import { playClick } from "@/lib/sound";
import { useDashboardWeek } from "@/hooks/useScoreMap";
import {
  TitanActionLink,
  TitanMetric,
  TitanPageHeader,
...
export default function Dashboard() {
  ...
  const weekData = useDashboardWeek();

  const engineCards: EngineCardModel[] = React.useMemo(() => {
    const pe = titan.perEngine;
    return (["body", "mind", "money", "charisma"] as const).map((key) => {
      const s = pe[key];
      return {
        key,
  ...
  const radarData = React.useMemo(() => {
    ...
    return [
      ...
      { subject: "General", score: pe.charisma.percent },
    ];
  }, [titan]);
